...
/** Minimum time on screen, so a fast load doesn't flash the splash. */
const MIN_VISIBLE_MS = 1600;
/**
 * Hard ceiling. A cold start predicts the whole slate and can take ~30s, and
 * holding a splash for that long feels broken — the dashboard has its own
 * spinner, so hand over and let the real UI show it is working.
 */
...
export const SplashScreen: React.FC<Props> = ({ isLoading, onFinished }) => {
  ...
  // Creep upward while loading; never reach the end on its own.
  useEffect(() => {
    if (!isLoading) return;
    const id = setInterval(() => {
      setProgress((p) => (p >= CREEP_CEILING ? p : p + Math.max(0.4, (CREEP_CEILING - p) * 0.06)));
    }, 90);
    return () => clearInterval(id);
  }, [isLoading]);

  // Leave regardless once the ceiling is reached, loaded or not.
  useEffect(() => {
    const fade = setTimeout(() => setLeaving(true), MAX_VISIBLE_MS - 250);
    const done = setTimeout(onFinished, MAX_VISIBLE_MS);
    return () => { clearTimeout(fade); clearTimeout(done); };
  }, [onFinished]);

  // Data arrived first: fill the bar, respect the minimum, then hand over.
  useEffect(() => {
    if (isLoading) return;
    setProgress(100);
    const elapsed = Date.now() - mountedAt.current;
    const wait = Math.max(MIN_VISIBLE_MS - elapsed, 350);
    const fade = setTimeout(() => setLeaving(true), wait - 250);
    const done = setTimeout(onFinished, wait);
    return () => { clearTimeout(fade); clearTimeout(done); };
