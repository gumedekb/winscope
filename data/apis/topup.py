...
def run(sa_history: bool = True, fixtures: bool = True,
        ...
        lookahead_days: int = 7) -> ApiResult:
    res = ApiResult()
    ...
    if api_football.available() and (sa_history or fixtures):
        if verbose:
    ...
    if fixtures and football_data_org.available():
        if verbose:
    ...
    if thesportsdb.available() and (sa_history or fixtures):
        if verbose:
            print("  TheSportsDB")
        try:
